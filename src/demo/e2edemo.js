...
(function (E2eSample) {
    ...
    function doPgpTest() {
        ...
        e2e.setup('super secret passphrase', 'Joe Test <joetest@example.com>').then(function () {
            log.debug('exporting public key');
            return e2e.exportKey();
        }).then(function (publicKey) {
            log.debug('encrypting/signing');
            return e2e.signEncrypt(buffer, publicKey, true).then(function (encryptedData) {
                return e2e.verifyDecrypt(encryptedData, publicKey);
            });
        }).then(function (result) {
            log.debug('decrypted!');
            var resultView = new Uint8Array(result.data);
            if (result.signedBy[0] == 'Joe Test <joetest@example.com>' && String.fromCharCode.apply(null, resultView) == String.fromCharCode.apply(null, byteView)) {
